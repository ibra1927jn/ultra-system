...
router.get("/status", async (req, res) => {
  try {
    ...
    console.error('❌ agentbus status error:', err.message);
...
router.post("/git-push", async (req, res) => {
  try {
    ...
    console.error('❌ agentbus git-push error:', err.message);
    res.status(500).json({ ok: false, error: 'Internal server error' });
  }
});
...
router.post("/send", async (req, res) => {
  try {
    ...
    console.error('❌ agentbus send error:', err.message);
    res.status(500).json({ ok: false, error: 'Internal server error' });
  }
});
...
router.post("/complete", async (req, res) => {
  try {
    ...
    console.error('❌ agentbus complete error:', err.message);
    res.status(500).json({ ok: false, error: 'Internal server error' });
  }
});
